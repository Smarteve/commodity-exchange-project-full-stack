...
class Main:
    def __init__(self):
        ...
        self.currency_dict: Dict[str,"Currency"] = {} # key currency name, value currency object 
    ...
    def add_currency(self,currency: "Currency") -> None:
        if currency.name in self.currency_dict:
            raise ValueError("f{currency.name} already existed, pls enter a new currency")
    ...
    def view_currency(self,name:str) -> str:
        print(self.currency_dict[name]) if name in self.currency_dict else print('commodity not found')
...
class Currency(Security):
    def __init__(self,name:str,prices:list["Price"],type:str,settlement_dates:list[date],tenor:str):
        super().__init__(name,prices)
        self.type = type 
    ...
    def __str__(self) -> str:
        settlement_dates_str = ", ".join([d.strftime("%Y-%m-%d") for d in self.settlement_dates])
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if action == "2":
            ...
            exchange_list = input(f"what is the exchange {name} is traded at(comma separated if multiple): ").split(",")
            prices = []
            ...
            try:
                ...
                print(e)
                continue
            main.get_exchange(exchange).add_commodity(name)# update self.commodities under Exchange class
            print(f"Thanks for adding {name}")
        if action == "3":
            #add new currency
            name = input("What is the name of the currency you want to add? ")
            settlement_dates_str = input("What is the settlement date of the commodity (yyyy-mm-dd)? comma separated if multiple ").split(",")
            settlement_dates = [_parse_time(time) for time in settlement_dates_str]
            type = input("what is the type of this currency, choose from spot/forward/swap ")
            tenor = input('what is the tenor of this currency (example: 3 years or 18 months) ')
            exchange_list = input(f"what is the exchange {name} is traded at(comma separated if multiple): ").split(",")
            prices = []
            ...
            try:
                ...
                print(e)
                continue
            main.get_exchange(exchange).add_currency(name)# update self.currencies under Exchange class
            print(f"Thanks for adding {name}")
