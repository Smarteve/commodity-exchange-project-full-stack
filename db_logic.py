...
def add_currency(name:str,type:str,tenor:str,settlement_dates:str,exchange_price_time_pairs:list[tuple]) -> None:
    helper.check_valid_exchanges(exchange_price_time_pairs)
    cursor = CONN.cursor
    try:
        cursor.execute("INSERT INTO Securities(name,type) Values(?,?)",(name,"currency"))
    except sqlite3.IntegrityError:
        raise DuplicateError(f"currency{name} already existed")

    security_id = cursor.lastrowid

    cursor.execute("INSERT INTO Currencies(id,type,tenor,settlement_dates) VALUES(?,?,?,?)",(security_id,type,tenor,settlement_dates))
    add_prices(security_id,exchange_price_time_pairs)
    CONN.commit()
